---
title: "No stress on Cres MTB Weekend"
slug: "no-stress-cres"
tag: "No stress"
tag_hot: false
date_label: "22.–24. maj 2026"
date_start: "2026-05-22"
date_end: "2026-05-24"
location: "Cres, Hrvaška"
location_meta: "Otok Cres · Kvarner"
audience: "Min. 6 oseb"
group_size: "6–12 udeležencev"
difficulty: "Srednje izkušeni in napredni"
title_short: "No stress Cres"
image: ""
image_label: "Otok, trail, kosilo, ponovitev."
description: "Mešano na žaru — skrite poti, planinske trase, rajski zalivi in jagnjetina v lokalnih oštarijah. Maja gremo zopet na \"no stress\" otok."
price_num: "140 €"
price_label: "Vodenje (brez nočitev)"
price_includes_accommodation: false
payment_url: ""
instructor: ""
gallery: []
included:
  - "Strokovno vodenje 3 dni"
  - "Načrtovanje poti glede na sposobnosti skupine"
  - "Pomoč pri organizaciji (priporočila za nastanitev, restavracije)"
  - "Dobre vibre in skupinska doživetja"
not_included:
  - "Prevoz do Cresa in trajekt (uredimo skupaj)"
  - "Nastanitev (pomagamo z rezervacijo)"
  - "Hrana in pijača"
  - "Osebno zavarovanje"
to_bring:
  - "Lastno MTB kolo (priporočljivo trail/all-mountain)"
  - "Čelado (obvezno)"
  - "Rokavice, kolesarska očala"
  - "Oblačila za 3 dni vožnje"
  - "Plavalna oprema (zalive si oglejmo tudi v živo)"
  - "Nahrbtnik s pitnikom"
itinerary:
  - day: "Petek"
    title: "Prihod in uvodna tura"
    content: "Po prihodu se nastanimo in skupaj odpeljemo krajšo uvodno turo — da pretegnemo noge in se spoznamo s terenom. Zvečer večerja v lokalni oštariji."
  - day: "Sobota"
    title: "Klasika Cresa"
    content: "Najlepša cela-dnevna tura po Cresu — kombinacija planinskih tras in skritih singletrailov. Kosilo v lokalni oštariji, popoldne pa še hitra kopalna postaja v enem od zalivov."
  - day: "Nedelja"
    title: "Še zadnji trail"
    content: "Krajša jutranja tura, čas za kosilo in slovo. Popoldne odhod nazaj proti Sloveniji."
registration_note: "Prijave do 10. maja 2026. Skupaj uredimo prevoz in nastanitev."
contact_email: "info@galaxysport.si"
---

## O izletu

Cres je za nas postal sinonim za "no stress" furanje — otok, kjer čas teče drugače. Tri dni kombiniramo skrite poti, planinske trase in rajske zalive, vmes pa kosila v lokalnih oštarijah, kjer postrežejo jagnjetino kot je še niste jedli.

Idealno za tiste, ki radi povezujejo MTB s počasnim odkrivanjem destinacije — ni hitenja, ni "kdo bo prvi". Gremo s tempom, ki dovoli, da uživaš v okolici.

## Kakšne so poti

Mešanica: nekaj makadamskih cest, veliko planinskih in pastirskih poti, nekaj tehnično zahtevnejših singletrailov. Vse poti prilagodimo skupini — če je kdo manj izkušen, izberemo lažje variante. Cilj je, da se vsi imajo lepo.
